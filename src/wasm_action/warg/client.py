...
class WargClient:
    def __init__(
        self, registry, warg_url, warg_token=None, warg_private_key=None, stealth=False
    ):
        ...
        self.private_key = (
            PrivateKey.load(warg_private_key) if warg_private_key else None
        )
    ...
    def get_checkpoint(self, namespace):
        return self.fetch_api.get_checkpoint(
    ...
    def get_content_sources(self, namespace, digest):
        return self.content_api.get_content_sources(
            digest=digest, warg_registry=self.get_warg_registry(namespace)
        )
